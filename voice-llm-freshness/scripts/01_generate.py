"""가상 음성 대화 로그 생성 (에피소드 단위)

- 빈 발화 = 오호출(앞에 사람 없음) + 사람 발화의 STT 누락. 두 비율은 운영 로그 57%에 맞춘 가정
- 세션은 기기가 닫음. fallback 뒤엔 대부분 새 세션, 무응답이면 계속 듣는 구조
- truth_* 4열은 07, 08 대조 전용
"""
...
P_EMPTY_MISFIRE = 0.78  # 오호출인데 원문이 남는 나머지는 대화 파편, TV 음성
