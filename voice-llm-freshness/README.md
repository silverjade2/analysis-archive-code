...
사전(주제 키워드, 제외 규칙, fallback 문구, 의도 규칙)과 상수는 전부 `common.py`. 02~06, 09는 `truth_` 컬럼을 읽지 않고 07, 08만 대조에 쓴다. `verify.py`가 그 순서와 02의 짝짓기(루프로 다시 계산)를 확인.

`logs.csv.gz`는 `row_id, session_id, ts(0.1초), log_type, text` + `truth_misfire, truth_topic, truth_fresh, truth_stale`. log_type 0 유저, 1 기기, 2 제어 코드, 3과 4는 schema에 없는 음성 원본 참조 행. 유저 발화의 text는 빈 문자열일 수 있음.
...
- 오호출은 유저 발화의 40% 안팎(생성 로그 실측 39.6%. 01의 파라미터 0.35는 에피소드 단위라 발화 단위와 다름), 그중 78%가 빈 발화. 사람 발화도 48%는 STT가 비움(이동형 기기 가정). 둘을 합쳐 운영 로그의 빈 발화 57%를 채움
- 사람도 파편을 말함. "아니", "이거 말고"는 사람이 기기에 한 말이고 의도 규칙에 안 걸림
- fallback 문구는 인식 실패 계열 14개 + 긴 꼬리 100개. 상위 두 문구 비중(39.8%, 17.6%)은 운영 로그에 맞춤
- 낡은 답: 최신 정보가 필요한 질문이 정상 응답을 받았고, 모델 cutoff(2025-01-01)부터 발화 시각까지가 주제별 유효 기간(3시간~720시간, lognormal 흔들림)을 넘으면 `truth_stale=1`. 시각과 날짜 질문은 기기 시계로 답한다고 보고 제외
...
- 생성기 가중치는 운영 로그의 주변 분포(빈 발화, 매칭 비율, fallback 문구 쏠림, 재시도율 순서)가 나올 때까지 손으로 조정한 값. 오호출 40%와 STT 누락 48%는 그 결과이고 추정치가 아님
...
- 세션을 넘어간 재시도는 기기 식별자가 없어 못 셈. 무응답 직후 재시도율 7.30%는 운영 로그보다 높음 (무응답이면 기기가 계속 듣는다는 가정이 강함)
