...
음성 비서 운영 로그에서 LLM의 낡은 답(knowledge cutoff 이후 바뀐 정보를 옛 정보로 답한 것)은 정상 응답으로 남아 직접 셀 수 없다. 원본 분석은 발화 쪽 신호(최신성 표현, 불만, 재시도)로 우회해 셌는데, 그 proxy가 실제로 몇 건을 가리키는지는 알 수 없었다. 여기서는 낡은 답에 `truth_stale`을 붙인 가상 로그를 만들어 proxy의 recall과 precision을 잰다. 실제 로그는 없고, 그림 1~3만 글에 적힌 운영 로그 비율(`data/record_rates.csv`, 손으로 옮김)을 그린 것.
...
10초 안쪽. Python 3.14.6(macOS), numpy 2.5.3, pandas 3.0.5, matplotlib 3.11.2, pillow 12.3.0. `data/*.csv.gz`는 gzip mtime을 0으로 고정해서 다시 돌려도 byte 동일. png, webp는 비교 안 함.
...
- 주제 키워드에 2글자 키워드와 부분 문자열 오탐(경기도, 사회복지사, 수영하고)을 넣음. 날씨 질문 대부분은 키워드 없이 들어옴("오늘 비 와")
