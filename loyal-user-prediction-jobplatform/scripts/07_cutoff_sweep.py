"""cutoff sweep. 위젯 CutoffExplorer 데이터

- v2 cutoff를 offset일만큼 이동. 양수면 타깃 결정 후 이벤트가 새어 들어감. snapshot = v1 + 선호 정보
- offset마다 06과 같은 모델/seed로 5-fold OOF AUC + 상위 5 gain importance (05처럼 one-hot 합산)
- 하한은 04와 같이 프로필 완성 전날, 상한 snapshot
"""
...
# 음의 offset은 거의 의미 없음. 하한이 프로필 전날이고 지연 중앙값 3일이라 -30이든 -1이든 대부분 같은 날. 위젯엔 그대로
