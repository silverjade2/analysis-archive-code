...
pipeline은 바꾸지 않는다. data/와 v1, v2 feature 테이블을 읽어 10이 그릴 작은 CSV를 쓴다. 다섯 부분이다.
...
import numpy as np
import pandas as pd
from common import CAT_COLS, DATA, N_DAYS, NOTEBOOK_FEATURES, PREF_FEATURES, RES, SEED, TARGET
    ...
    rows.append(
        {
            "user": int(u),
            "consented": int(pos[u]),
            "season_joiner": int(season[u]),
            "test_day": int(users["test1_day"][u] - join[u]),
            "profile_day": int(profile[u] - join[u]),
            "consent_day": int(consent[u] - join[u]) if pos[u] else -1,
            "v2_cutoff_day": int(cutoff[u] - join[u]),
            "logins_365d": len(ld),
            "logins_after_cutoff_365d": int((ld > cutoff[u] - join[u]).sum()),
...
).round(6).to_csv(RES / "oof_scores.csv", index=False)
rows = []
for label, s in curves.items():
...
for vname, s in scores.items():
    order = neg[np.argsort(-s[neg])]  # 06과 같은 정렬
...
for vname, key in [("v1", "v1 top 10% (snapshot score)"), ("v2", "v2 top 10% (time-cut score)")]:
    ...
    assert abs(top - nudge[key]) < 1e-4, (vname, top, nudge[key])  # 1분위 == 06의 넛지 리스트
...
overlap_ref = pd.read_csv(RES / "nudge_list_overlap.csv").set_index("metric")["value"][
    "overlap between v1 and v2 top-10% lists"
]
    ...
    rows.append(
        {
            "quadrant": qname,
            "n": len(r),
            "share_of_list": len(r) / k,
            "mean_truth_p": truth_p[r].mean(),
