"""02 — Journey map: where do users stop?

Reproduces the three findings that came BEFORE any modeling:
  (1) status funnel of the full population — most users stop right after joining;
  (2) among users who took the test and filled the required profile, the preference fields
      are what's left blank (salary left at default, few welfare items picked);
  (3) dormancy — a large share has not logged in for 6–12 months, and returns cluster
      around recruiting seasons.
"""
import sys
...
from common import DATA, FIG, RES, SNAPSHOT, SERVICE_START, SEASONS, N_DAYS
...
cnt = pop["status"].value_counts().reindex(order)
share = cnt / cnt.sum()
consent_rate = users["matching_use_yn"].mean()
funnel = pd.DataFrame({"status": labels, "users": cnt.values, "share": share.values.round(4)})
funnel.loc[len(funnel)] = ["  └ consented (core users)", int(round(cnt.iloc[-1] * consent_rate)),
...
pref = pd.DataFrame({
    ...
    "share": [(users["pref_salary_default_yn"] == "Y").mean(), (users["pref_welfare_cnt"] <= 1).mean(),
              (users["pref_welfare_cnt"] < 5).mean(), users["truth_pref_complete"].mean()],
}).round(4)
by_consent = users.groupby("matching_use_yn").agg(
    salary_default=("pref_salary_default_yn", lambda s: (s == "Y").mean()),
    welfare_lt5=("pref_welfare_cnt", lambda s: (s < 5).mean()),
    pref_complete=("truth_pref_complete", "mean")).round(4)
...
weekly = pd.Series(logins.sum(0), index=days).resample("W").sum()[:-1]   # drop partial last week
...
ax.set_title("Returns cluster around recruiting seasons and, for consented users, after consent", loc="left", fontsize=11)
