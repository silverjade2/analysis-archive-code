"""06 — Nudge list comparison.

The operational use of the model was a weekly list: score users who have NOT consented, take
the top decile, hand it to marketing. This script builds that list twice — with v1 (snapshot)
scores and with v2 (time-cut) scores — using out-of-fold LightGBM probabilities, and asks who
ends up on each list.

Because the data is synthetic we also know each user's true consent propensity
(truth_p_consent), so the two lists can be scored against the ground truth: a good nudge list
should be full of users who are *close* to converting, not users who are merely active.
"""
import sys
...
def describe(idx, label):
    u = users.iloc[idx]
    f = v1.iloc[idx]  # snapshot facts about who they are today
    return {
        "list": label,
        "n": len(idx),
...
# random-list baseline for the truth metric
