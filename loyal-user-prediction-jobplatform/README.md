...
원본 프로젝트(2023.07)는 구직 플랫폼 유저 45만 명을 저니맵으로 나누고, 프로필·검사를 마친 유저 중 누가 기업 추천에 동의하는지(핵심 유저 전환)를 분류 모델로 예측했다. 이 폴더는 그 pipeline을 같은 스키마의 가상데이터로 다시 돌리되, 당시 모델의 구조적 문제(타깃이 결정된 이후의 행동이 feature에 포함됨)를 데이터에 그대로 심어 놓고, 시점을 절단한 두 번째 버전과 나란히 채점한다.
