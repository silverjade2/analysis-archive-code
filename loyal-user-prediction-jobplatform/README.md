...
- 01의 상태 비율(가입만 80 / 검사만 9 / 프로필만 3)은 글의 상태 분포와 같게 잡음. 모델링 대상 38,355명을 뽑는 주 rng를 건드리면 users.csv가 통째로 바뀌어서 status 3은 고정하고 나머지 41만 명만 별도 스트림(`SEED + 1`)으로 재배정. population.csv만 바뀌고 users.csv와 결과 CSV는 그 전과 byte 동일
