...
기기 500대 x 180일 가상 로그에 전조 신호 2개와 함정 1개를 넣고, 원본(실제 기기 로그의 심각 이벤트 전조 예측, 양성 0.36%)과 같은 절차로 모델이 무엇을 학습하는지 본다. 원본에서 결과를 가른 건 불균형 처리가 아니라 분할, 지표, feature 검증 설계였고 그 순서를 그대로 따른다. 실제 로그는 없음.
...
35초 정도 (05 학습 12초, 06 SHAP 11초).

Python 3.14, numpy 2.5, pandas 3.0, scikit-learn 1.9, LightGBM 4.7, shap 0.52, matplotlib 3.11. fig5만 shap 라이브러리가 그리는 그림이라 픽셀이 조금 다를 수 있음.
...
| 02 | 넣은 신호가 들어갔는지 이벤트일 기준 정렬로 확인 | `fig1~3`, `signal_validation.csv` |
| 03 | device-day feature 33개 + target (t+1~t+7 심각 이벤트) | `data/features.csv`, `feature_table_stats.csv` |
| 04 | leakage 검증 3종. point-in-time recomputation check, target 정의, positive control | `leakage_checks.csv` |
| 05 | time-based split (day 150부터 test, purge 7일) + LightGBM 불균형 처리 3종 | `model_compare.csv`, `split_stats.csv`, `fig4`, `data/model_*.joblib` (미커밋) |
...
- 04: point-in-time recomputation check 불일치 0건. positive control 34건은 일부러 넣은 leakage를 검증이 잡는다는 뜻, 여기가 0이면 검증 쪽이 망가진 것
- 05: no_handling PR-AUC 0.609. 무작위 기준선 0.004의 150배쯤이고 불균형 처리 2종보다 높음
- 06: W7 제거 ablation 차이 -0.007. 함정은 빼도 성능 그대로

## 데이터

`device_events.csv` 251,914행. long 포맷으로 count > 0인 날만 기록. 열은 `device_id`(D0000~D0499), `event_date`(2025-01-01부터 180일), `event_type`(usage / warning / severe), `event_code`(U1, W1~W9, S1), `count`. `ground_truth.csv` 500행은 `group`(normal / severe / confounder)과 `severe_event_date`(severe 40대만). 평가와 검증에만 쓴다.
...
- 넣은 신호 중 사용량 표준편차는 SHAP 1위인데 W3 계열은 7, 22, 25위이고 함정 `w7_cnt_7d`가 5위. 08로 본 원인은 라벨 창. 양성 행은 이벤트 1~7일 전인데 W3 램프가 7일 전에 시작해 6~7일 전 행의 `w3_cnt_7d`는 음성 평균과 비슷하고 1일 전 행에서야 15.4 (`w3_by_horizon.csv`). 단일 feature AUC는 `w3_cnt_7d` 0.868, `u1_std_7d` 0.877로 비슷한데도 SHAP 순위가 벌어진 이유는 확인 안 함 (`single_feature_auc.csv`)
