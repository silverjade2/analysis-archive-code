...
pd.DataFrame({
    "metric": ["train_rows", "train_day_max", "train_positive", "test_rows", "test_day_min", "test_positive",
               "scale_pos_weight"],
    "value": [len(train), int(train["day"].max()), int(y_tr.sum()), len(test), TEST_START_DAY, int(y_te.sum()),
              round(n_neg / n_pos, 2)],
