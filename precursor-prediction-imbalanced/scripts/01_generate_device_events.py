# 가상데이터 생성: 기기 500대 × 180일의 사용량·경고·심각 이벤트, 전조 신호 3종(W3 램프, 사용량 분산, W7 교란)을 심음
...
# 경고 코드별 기본 발생률(일 평균 건수), W3/W7에 신호가 얹힘
WARNING_BASE_RATES = {
...
# 사용량(U1): Poisson(mu * eps), eps는 lognormal(평균 1 보정)
# 신호 (2): 이벤트 전 14일 동안 sigma만 선형으로 키움 → 평균 유지, 분산 증가
...
# 정답지: 검증 플롯·성능 평가용, 모델 입력으로는 쓰지 않음
