...
pd.DataFrame({
    "metric": ["recompute_sample_rows", "recompute_n_features", "recompute_mismatch",
               "post_event_rows", "target_definition_mismatch",
               "negative_control_rows", "negative_control_detected"],
    "value": [N_SAMPLE, len(feature_cols), mismatch, post_event_rows, target_mismatch, 50, leak_detected],
}).to_csv(res_dir / "leakage_checks.csv", index=False)
