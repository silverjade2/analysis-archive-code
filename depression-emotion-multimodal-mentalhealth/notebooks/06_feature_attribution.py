"""06. 해석. 원 프로젝트는 SHAP 텍스트 플롯 예문 1건으로 '슬픔 ← 외로운·무의미'를 보였다.
여기서는 (1) 감정별 전역 상위 피처, (2) 예문에서 본 토큰이 실제 sadness 발화 중 몇 %에 있는지,
(3) 융합 분류기가 텍스트/음성 채널에 준 가중치 합을 기록한다."""
import numpy as np
import pandas as pd
from sklearn.model_selection import GroupShuffleSplit
from common import SEED, DATA, RESULTS, FIGURES, EMOTIONS, AUDIO_COLS, TEST_SIZE, setup_mpl
from models import TextClf, MultimodalClf
...
for e in EMOTIONS:
    ...
    for f, c in feats[e]:
        ...
        seen.add(key); rows.append(dict(emotion=e, rank=k + 1, feature=f, coef=c)); k += 1
        if k == 8: break
pd.DataFrame(rows).to_csv(RESULTS / "emotion_top_features_by_class.csv", index=False)

# (2) 예문 토큰의 전역 커버리지: sadness 발화 중 '외롭'/'무의미' 포함 비율, 그리고 그 토큰이 있을 때 sadness 비율
tok = ["외롭", "무의미"]
cov = []
for t in tok:
    has = daily.text.str.contains(t)
    cov.append(dict(token=t,
...
cov = pd.DataFrame(cov).round(4); cov.to_csv(RESULTS / "shap_example_token_coverage.csv", index=False)

# (3) 융합 가중치
mm = MultimodalClf(SEED).fit(tr.text, tr[AUDIO_COLS].values, tr.emotion_label)
t_norm, a_norm = mm.channel_coef_norm()
pd.DataFrame([dict(channel="text", abs_coef_sum=t_norm, share=t_norm / (t_norm + a_norm)),
              dict(channel="audio", abs_coef_sum=a_norm, share=a_norm / (t_norm + a_norm))]).round(4)\
    .to_csv(RESULTS / "fusion_channel_weight.csv", index=False)

# 혼동행렬: 텍스트 / 멀티모달 (화자 분리, 전체) 과 그 차이
from sklearn.metrics import confusion_matrix
from common import C_TEXT, C_MM, C_AUDIO, C_NEG, C_GRAY, C_DARK, EMO_COLORS
...
pd.DataFrame([dict(target=e, diag_change=int(diff.loc[e, e]), pulled_from_other_classes=int(diff[e].drop(e).sum())) for e in EMOTIONS]
...
# fig6: 왼쪽 레이더(감정별 recall), 오른쪽 혼동행렬 차이 히트맵
...
for col, c, lab in [("text_recall", C_TEXT, "텍스트"), ("multimodal_recall", C_AUDIO, "멀티모달")]:
    ...
    ax.plot(ang, v, color=c, lw=2, label=lab); ax.fill(ang, v, color=c, alpha=0.15)
ax.set_xticks(ang[:-1]); ax.set_xticklabels(EMOTIONS, fontsize=9)
ax.set_ylim(0.6, 0.95); ax.set_yticks([0.7, 0.8, 0.9]); ax.set_yticklabels(["0.7", "0.8", "0.9"], fontsize=7)
ax.legend(loc="lower left", bbox_to_anchor=(-0.15, -0.1), fontsize=9); ax.set_title("감정별 recall (화자 분리, 전체)", pad=14)
...
for i in range(7):
    for j in range(7):
        v = diff.values[i, j]
        if v != 0: ax2.text(j, i, f"{v:+d}", ha="center", va="center", fontsize=7.5, color="white" if abs(v) > lim * 0.55 else C_DARK)
ax2.set_xlabel("예측"); ax2.set_ylabel("정답"); ax2.set_title("혼동행렬 차이: 멀티모달 − 텍스트 (건수)")
plt.colorbar(im, ax=ax2, fraction=0.046, pad=0.03)
plt.tight_layout(); plt.savefig(FIGURES / "fig6_audio_gain_by_class.png"); plt.close()

# fig8: SHAP 텍스트 플롯을 흉내낸 로컬 토큰 기여 vs 전역 상위 피처.
# 로컬 기여 = 각 char n-gram의 (tfidf 값 × sadness 계수)를 토큰(어절) 단위로 합산.
ex_pool = daily[daily.text.str.contains("외롭") & daily.text.str.contains("무의미")]
if len(ex_pool) == 0:
    ex_pool = daily[daily.text.str.contains("외롭")]
ex = ex_pool.iloc[0]
vec, clf = m.vec, m.clf
names = vec.get_feature_names_out(); sad_i = list(clf.classes_).index("sadness")
...
local["example_text"] = ex.text; local["true_emotion"] = ex.emotion_label; local["truth_depressed"] = ex.truth_depressed
local.to_csv(RESULTS / "shap_like_local_example.csv", index=False)
glob_sad = pd.read_csv(RESULTS / "emotion_top_features_by_class.csv"); glob_sad = glob_sad[glob_sad.emotion == "sadness"].copy()
...
for t, c in tok_contrib:
    ...
    ax1.text(x + w / 2, 0.55, t, ha="center", va="center", fontsize=10, color="white" if abs(c) / lim > 0.45 else C_DARK)
...
ax1.set_title(f"로컬: 예문 한 건의 토큰별 sadness 기여 (정답 감정 = {ex.emotion_label}, truth_depressed = {ex.truth_depressed})", fontsize=10, loc="left")
# 아래: 전역 상위 피처
...
ax2.set_xlabel("LR coef (sadness, char n-gram)"); ax2.set_title("전역: sadness 상위 피처 8개 — '외롭', '무의미'는 없다", fontsize=10, loc="left")
ax2.spines[["top", "right"]].set_visible(False)
plt.tight_layout(); plt.savefig(FIGURES / "fig8_local_vs_global.png"); plt.close()
print(local.to_string()); print(diff)
