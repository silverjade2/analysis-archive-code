...
import numpy as np
import pandas as pd
from common import (SEED, DATA, EMOTIONS, EMOTION_P, N_DAILY, N_COUNSEL_SYMPTOM,
                    N_COUNSEL_NORMAL, N_SPEAKERS, AUDIO_COLS)
...
DEPRESSED = ["무의미하", "외롭", "무기력하", "우울하", "죽고 싶", "눈물이 나", "잠이 안 오",
             "입맛이 없", "희망이 없", "지쳤", "숨이 막히", "다 포기하고 싶", "가라앉",
             "의욕이 없", "쓸모없", "아무것도 하기 싫", "혼자인 것 같"]
NORMAL_LIFE = ["여행 가", "맛집 갔", "주말에 쉬", "친구 만났", "운동했", "영화 봤", "강아지 산책",
               "날씨 좋", "회사 갔", "프로젝트 끝났", "밥 먹었", "카페 갔", "책 읽었", "청소했",
               "장 봤", "부모님 뵀", "시험 봤", "이사했"]
EMOTION_TOKENS = {
...
SYMPTOM_INTENT = ["정신증상/초조함", "정신증상/불안", "정신증상/무기력", "정신증상/기분우울",
...
# 화자 지문은 음색 계열 피처(MFCC 7~13, chroma, duration)에 실린다. 감정 오프셋은 에너지·피치 계열에.
...
for i in range(N_COUNSEL):
    ...
    rows.append(dict(
        utt_id=f"c{i:05d}", source="counsel", speaker_id=None, intent=intent,
        emotion_label=None, text=build_text("counsel", truth_dep, None, inf_dep, None),
        truth_depressed=truth_dep, truth_text_informative_dep=inf_dep,
...
for i in range(N_DAILY):
    ...
    rows.append(dict(
        utt_id=f"d{i:05d}", source="daily", speaker_id=f"spk{s:03d}", intent=None,
        emotion_label=emotion, text=text,
        truth_depressed=truth_dep, truth_text_informative_dep=inf_dep,
        truth_text_informative_emo=inf_emo, truth_audio_informative=inf_audio,
    ))
    audio_rows.append(a)
