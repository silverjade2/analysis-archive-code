...
import numpy as np
import pandas as pd
from common import AUDIO_COLS, DATA, EMOTIONS, RESULTS, SEED
from matplotlib.lines import Line2D
from matplotlib.patches import Patch, Rectangle
from sitestyle import (
    BLUE,
    CAT8,
    DARK,
    DIVERGING,
...
ax.legend(
    handles=[Patch(color=BLUE, label="학습에 사용"), Patch(color=ORANGE, label="학습에서 제외")],
    loc="upper center",
...
ax.legend(
    handles=[
        Line2D([], [], color=GRAY, lw=2.5, label="테스트셋이 다른 비교"),
...
ax1.set_xscale("log")
ax1.set_ylabel("accuracy (seed 3개, 밴드 = ±1 std)")
...
ax.set_xticks(ang[:-1])
ax.set_xticklabels(EMOTIONS, fontsize=9, color=DARK)
ax.set_ylim(0.6, 0.95)
ax.set_yticks([0.7, 0.8, 0.9])
ax.set_yticklabels(["0.7", "0.8", "0.9"], fontsize=7, color=MUTED)
...
for i in range(7):
    for j in range(7):
        ...
        if v != 0:
            ax2.text(
                j, i, f"{v:+d}", ha="center", va="center", fontsize=7.5, color="white" if abs(v) > lim * 0.55 else DARK
            )
ax2.set_xlabel("예측")
ax2.set_ylabel("정답")
...
axes[0].legend(
    handles=[Line2D([], [], marker="o", ls="", color=c, label=s) for s, c in spk_colors.items()],
    fontsize=7,
    loc="lower right",
    ncol=2,
)
...
ax1.axis("off")
ax1.set_title(
    f"local: 예문 한 건의 토큰별 sadness 기여 (정답 감정 = {true_emo}, truth_depressed = {true_dep})", fontsize=10.5
)
