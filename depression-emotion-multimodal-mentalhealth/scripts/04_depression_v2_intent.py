...
from common import SEED, DATA, RESULTS, FIGURES, depression_split, setup_mpl
...
for name, train in [("v1_exclude_normal", tr_v1), ("v2_include_normal", tr)]:
    ...
    pred = m.predict(te.text); proba = m.predict_proba(te.text)[:, 1]
    for subset, mask in [("test_all", np.ones(len(te), bool)), ("test_counsel_only", (te.source == "counsel").values),
                         ("test_daily_only", (te.source == "daily").values)]:
        rows.append(dict(version=name, scoring="truth_depressed", subset=subset, n=int(mask.sum()), n_train=len(train),
                         auc=roc_auc_score(te.truth_depressed[mask], proba[mask]), **metrics(te.truth_depressed[mask], pred[mask])))
    if name == "v2_include_normal":
        feats = m.top_features(40)
        pd.DataFrame([dict(direction="depressed(+)", feature=f, coef=c) for f, c in feats["pos"]]
...
res = pd.DataFrame(rows).round(4); res.to_csv(RESULTS / "depression_v1_v2_compare.csv", index=False)
print(res.to_string())
...
ax.axvline(orig, color=C_TEXT, ls="--", lw=1.5); ax.text(orig, 2.55, f"v1 · 원 범위\n학습 라벨 기준 {orig:.3f}", ha="center", fontsize=8, color=C_DARK)
ax.set_xlim(0.78, 1.02); ax.set_ylim(-0.5, 3.0); ax.set_xlabel("accuracy (실제 우울 기준)")
ax.legend(loc="lower left", fontsize=8); ax.spines[["top", "right"]].set_visible(False)
...
def kind(f):
    t = f.strip()
    if any(t == e or t.endswith(e) and len(t) <= len(e) + 1 for e in ENDINGS): return "어미"
    if any(t == w or (t and w.startswith(t) and len(t) >= 2) or t in w for w in IDIOM): return "관용어"
    return "내용"
...
v1f = dedupe(pd.read_csv(RESULTS / "depression_v1_top_features.csv")); v1f = v1f[v1f.direction.str.contains(r"\+")].head(12)
v2f = dedupe(pd.read_csv(RESULTS / "depression_v2_top_features.csv")); v2f = v2f[v2f.direction.str.contains(r"\+")].head(12)
v1f = v1f.assign(kind=v1f.feature.map(kind)); v2f = v2f.assign(kind=v2f.feature.map(kind))
pd.concat([v1f.assign(version="v1"), v2f.assign(version="v2")])[["version", "feature", "coef", "kind"]].to_csv(RESULTS / "depression_feature_kinds.csv", index=False)
fig, ax = plt.subplots(figsize=(10, 4.6))
for i, (_, r) in enumerate(v1f.iterrows()):
    ax.barh(11 - i, -r.coef, color=KC[r.kind]); ax.text(-r.coef - 0.15, 11 - i, repr(r.feature.strip()), ha="right", va="center", fontsize=9)
for i, (_, r) in enumerate(v2f.iterrows()):
    ax.barh(11 - i, r.coef, color=KC[r.kind]); ax.text(r.coef + 0.15, 11 - i, repr(r.feature.strip()), ha="left", va="center", fontsize=9)
ax.axvline(0, color=C_DARK, lw=1); ax.set_yticks([]); ax.set_xlim(-12, 12)
ax.set_xticks([-8, -4, 0, 4, 8]); ax.set_xticklabels(["8", "4", "0", "4", "8"]); ax.set_xlabel("|LR coef| (char n-gram, 우울 방향)")
ax.text(-6, 12.3, "v1 · 상담 일상 의도 제외", ha="center", fontsize=10); ax.text(6, 12.3, "v2 · 상담 일상 의도 = 0", ha="center", fontsize=10)
from matplotlib.patches import Patch
ax.legend(handles=[Patch(color=c, label=k) for k, c in KC.items()], loc="lower right", fontsize=9, title="토큰 종류")
ax.spines[["top", "right", "left"]].set_visible(False); ax.set_ylim(-0.8, 13)
plt.tight_layout(); plt.savefig(FIGURES / "fig3_depression_features.png"); plt.close()
