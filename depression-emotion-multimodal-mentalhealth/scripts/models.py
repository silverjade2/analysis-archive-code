...
class TextClf:
    ...
    def fit(self, texts, y):
        self.clf.fit(self.vec.fit_transform(texts), y)
        return self
    ...
    def top_features(self, k=15):
        ...
        if coef.shape[0] == 1:
            ...
            return {
                "pos": list(zip(names[order[::-1][:k]], coef[0][order[::-1][:k]].round(3))),
                "neg": list(zip(names[order[:k]], coef[0][order[:k]].round(3))),
            }
        return {
            c: list(zip(names[np.argsort(coef[i])[::-1][:k]], np.sort(coef[i])[::-1][:k].round(3)))
...
class MultimodalClf:
    ...
    def fit(self, texts, A, y):
        ...
        A = np.asarray(A)
        y = np.asarray(y)
        self.text = TextClf(self.seed)
        self.audio = AudioClf(self.seed)
        oof_t = self._oof(lambda: TextClf(self.seed), texts, y)
        oof_a = self._oof(lambda: AudioClf(self.seed), A, y)
        self.text.fit(texts, y)
        self.audio.fit(A, y)
        self.fuser.fit(np.hstack([oof_t, oof_a]), y)
        return self
