...
from common import SEED, DATA, RESULTS, FIGURES, EMOTIONS, AUDIO_COLS, N_MULTIMODAL, TEST_SIZE, setup_mpl
from models import TextClf, AudioClf, MultimodalClf
...
for model in ["multimodal", "speaker_prior"]:
    ...
    rows.append(dict(protocol="a_original", data="sub_2000", split="random", model=model, n_train=len(tr_s), n_test=len(te_s), accuracy=r["accuracy"], f1_weighted=r["f1_weighted"]))

# (b) 같은 2,000건, 같은 분할
for model in ["text", "audio", "multimodal"]:
    r = run(tr_s, te_s, model, SEED)
    rows.append(dict(protocol="b_same_subset", data="sub_2000", split="random", model=model, n_train=len(tr_s), n_test=len(te_s), accuracy=r["accuracy"], f1_weighted=r["f1_weighted"]))
...
for data_name, d in [("all_19374", daily), ("sub_2000", sub2000)]:
    ...
    for model in ["text", "audio", "multimodal", "speaker_prior"]:
        ...
        rows.append(dict(protocol="c_speaker_split", data=data_name, split="speaker", model=model, n_train=len(tr), n_test=len(te), accuracy=r["accuracy"], f1_weighted=r["f1_weighted"]))
...
for name, te in [("all_19374_random", te_all), ("sub_2000_random", te_s), ("all_19374_speaker", te_c_all)]:
    ...
    rows.append(dict(protocol="oracle", data=name, split=name.split("_")[-1], model="oracle", n_train=0, n_test=len(te), accuracy=accuracy_score(te.emotion_label, op), f1_weighted=f1_score(te.emotion_label, op, average="weighted")))
...
for e in EMOTIONS:
    ...
    per.append(dict(emotion=e, n_test=int(m.sum()),
                    text_recall=(pred_c_text[m] == e).mean(), multimodal_recall=(pred_c_mm[m] == e).mean()))
per = pd.DataFrame(per); per["gain"] = per.multimodal_recall - per.text_recall
...
arrows = [
    ("(a) 원 방식\n텍스트 전체 → 멀티모달 2,000\n(테스트셋이 다름)", g("a_original", "all_19374", "text"), g("a_original", "sub_2000", "multimodal"), f"n={g('a_original','all_19374','text','n_test'):,} vs {g('a_original','sub_2000','multimodal','n_test'):,}", C_GRAY),
    ("(b) 같은 2,000건 · 랜덤", g("b_same_subset", "sub_2000", "text"), g("b_same_subset", "sub_2000", "multimodal"), f"n={g('b_same_subset','sub_2000','text','n_test'):,}", C_MM),
    ("(c) 같은 2,000건 · 화자 분리", g("c_speaker_split", "sub_2000", "text"), g("c_speaker_split", "sub_2000", "multimodal"), f"n={g('c_speaker_split','sub_2000','text','n_test'):,}", C_AUDIO),
    ("전체 · 랜덤", g("a_original", "all_19374", "text"), g("c_random_all", "all_19374", "multimodal"), f"n={g('c_random_all','all_19374','multimodal','n_test'):,}", C_MM),
    ("(c) 전체 · 화자 분리", g("c_speaker_split", "all_19374", "text"), g("c_speaker_split", "all_19374", "multimodal"), f"n={g('c_speaker_split','all_19374','text','n_test'):,}", C_AUDIO),
]
...
style = {("random", "text"): (C_TEXT, "--"), ("random", "multimodal"): (C_MM, "--"),
         ("speaker", "text"): (C_TEXT, "-"), ("speaker", "multimodal"): (C_AUDIO, "-")}
for (how, model), (c, ls) in style.items():
    a = agg[(agg.split == how) & (agg.model == model)]
    ax1.plot(a.n, a["mean"], color=c, ls=ls, marker="o", ms=4, label=f"{model} · {'랜덤' if how == 'random' else '화자 분리'}")
    ax1.fill_between(a.n, a["mean"] - a["std"], a["mean"] + a["std"], color=c, alpha=0.15)
ax1.set_xscale("log"); ax1.set_ylabel("accuracy (시드 3개, 밴드 = ±1 std)"); ax1.legend(fontsize=8, ncol=2, loc="lower right")
ax1.axvline(2000, color=C_GRAY, ls=":", lw=1); ax1.text(2000, ax1.get_ylim()[1] - 0.004, " 원 프로젝트의 멀티모달 N", fontsize=8, color=C_GRAY, va="top")
ax1.spines[["top", "right"]].set_visible(False)
for how, c in [("random", C_MM), ("speaker", C_AUDIO)]:
    gg = gap[gap.split == how]
    ax2.plot(gg.n, gg.gain, color=c, marker="o", ms=4, ls="--" if how == "random" else "-", label="랜덤 분할" if how == "random" else "화자 분리")
ax2.axhline(0, color=C_DARK, lw=0.8); ax2.set_ylabel("멀티모달 − 텍스트"); ax2.set_xlabel("학습에 쓴 발화 수 N (log)")
ax2.legend(fontsize=8); ax2.spines[["top", "right"]].set_visible(False)
ax1.set_title("N과 분할 방식에 따른 accuracy, 그리고 음성이 더한 것")
plt.tight_layout(); plt.savefig(FIGURES / "fig5_n_curve.png"); plt.close()
