...
style = df.groupby("source").agg(n=("utt_id", "size"), mean_char=("n_char", "mean"),
                                 polite_rate=("polite_ending", "mean"),
                                 truth_depressed_rate=("truth_depressed", "mean")).round(3)
...
# 화자 편중: 화자별 최빈 감정 비율
spk = daily.groupby("speaker_id").emotion_label.agg(n="size", top_share=lambda s: s.value_counts(normalize=True).iloc[0])
...
ax = axes[0]
W = {"counsel": len(counsel), "daily": len(daily)}; total = sum(W.values()); x = 0
for src in ["counsel", "daily"]:
    ...
    for _, r in sub.iterrows():
        ...
        ax.text(x + w / 2 - 0.01, y + h / 2 - 0.01, f"{r.label}\n{r.n:,}건" + ("\n학습에서 제외" if not r.used_in_training else ""),
                ha="center", va="center", color="white", fontsize=9)
        y += h
    if src == "daily":
        ax.add_patch(plt.Rectangle((x, 0.985), w - 0.02, 0.0, color="none"))
        ax.text(x + w / 2 - 0.01, 1.03, "라벨 1 · 0건\n(이 칸이 비어 있다)", ha="center", va="bottom", color=C_DARK, fontsize=9)
    ax.text(x + w / 2 - 0.01, -0.06, "상담 스크립트" if src == "counsel" else "일상 대화", ha="center", va="top", fontsize=10)
    x += w
...
pd.DataFrame(dict(component=["PC1", "PC2"], explained_variance_ratio=ev.round(4))).to_csv(RESULTS / "eda_audio_pca.csv", index=False)
...
for ax in axes:
    ax.set_xlabel(f"PC1 ({ev[0]:.0%})"); ax.spines[["top", "right"]].set_visible(False)
...
plt.tight_layout(); plt.savefig(FIGURES / "fig7_audio_pca_speaker_vs_emotion.png"); plt.close()
print(style); print(spk_summary); print(cells)
