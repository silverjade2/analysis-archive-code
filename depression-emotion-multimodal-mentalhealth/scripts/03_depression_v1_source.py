...
from common import SEED, DATA, RESULTS, depression_split
...
rows.append(dict(scoring="truth_depressed", subset="test_as_original", n=len(te_v1), **metrics(te_v1.truth_depressed, p_v1)))
...
c1 = te[(te.source == "counsel") & (te.is_symptom == 1)]; p_c1 = p_all[((te.source == "counsel") & (te.is_symptom == 1)).values]
detail = pd.DataFrame([
    dict(group="counsel & normal intent (excluded from training, all 4,000)", n=len(dropped), pred_depressed_rate=p_dropped.mean()),
    dict(group="counsel & symptom intent (test)", n=len(c1), pred_depressed_rate=p_c1.mean()),
    dict(group="daily & truth_depressed=1 (test)", n=int((d.truth_depressed == 1).sum()), pred_depressed_rate=p_d[(d.truth_depressed == 1).values].mean()),
...
pd.DataFrame([dict(subset="test_all", oracle_accuracy=accuracy_score(te.truth_depressed, op),
                   oracle_f1=f1_score(te.truth_depressed, op))]).round(4).to_csv(RESULTS / "depression_oracle.csv", index=False)
...
print(res.to_string()); print(detail.to_string()); print("top:", [f for f, _ in feats["pos"][:12]])
