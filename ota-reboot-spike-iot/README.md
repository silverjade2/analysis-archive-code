...
| 03 | robust z로 스파이크 검출, 신규/재활성/기존 분해, 요일과 시작 시각 프로파일 | `spike_days.csv`, `spike_decomposition.csv`, `hypothesis_table.csv` |
...
03은 `is_ota`, `truth_*`를 읽지 않는다. 읽는 건 채점 04와 그림 05(OTA일 점 찍기, 경로별 히스토그램)뿐.
...
- 04: OTA 직후 세션 중 재개 58~64%, 오류 가동 35~41%. 생성기에 넣은 값이지 실제가 아님
