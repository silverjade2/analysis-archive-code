...
fleet 일별 KPI의 스파이크가 전부 펌웨어 OTA 배포일과 겹쳤고, 급증한 세션은 사용자가 아니라 재부팅이 만든 것이었다는 운영 분석의 재현. 기기 2,000대 세션 로그를 가상으로 만들어 OTA 4회를 넣고, 당시 판정 규칙(첫 시작 시각 집중 비율 0.5)이 그 4일을 검출하는지 평가한다. 실제 로그는 없고 판정 기준 값만 운영 분석에서 그대로 옮김.
...
1분 안쪽. numpy 2.4.4, pandas 3.0.2, matplotlib 3.10.8. Linux(Python 3.12.3)와 macOS(3.14.6)에서 결과 CSV가 byte 단위로 같았음. png는 폰트 메타데이터 때문에 비교 안 함.

## 스크립트

| | 역할 | 주요 출력 |
|---|---|---|
| 01 | 기기, 세션 로그 생성. 올타임 25%, 스케줄 5%, 설치 배치 2회, OTA 4회를 넣음 | `data/devices.csv`, `sessions.csv`, `ota_days.csv` |
...
| 04 | ground truth 대비 평가. 임계 민감도, carry-over 비교, OTA 직후 세션의 경로 비중 | `threshold_sensitivity.csv`, `path_share.csv`, `summary.csv` |
| 05 | 그림 | `outputs/figures/fig1~5` |

03의 판정 기준(MAD 기준 z 3.0, 신규 3일, 재활성 7일 간격, 집중 비율 0.5)과 04의 15분 창은 운영 분석에서 쓴 값을 그대로 옮긴 것. 03은 `is_ota`, `truth_*`를 읽지 않고 읽는 건 평가 04와 그림 05(OTA일 점 찍기, 경로별 히스토그램)뿐.
