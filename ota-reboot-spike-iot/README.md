...
원본은 실제 fleet의 일별 KPI 스파이크 원인 분석. 스파이크일은 전부 펌웨어 OTA 배포일이었고, 튄 세션은 사람이 아니라 재부팅이 만든 것이었다. 여기서는 기기 2,000대의 세션 로그를 가상으로 만들어 OTA 배포일 4개를 심고, 당시 쓴 판정 규칙(첫 시작 시각 집중 비율 0.5)이 그 4일을 잡는지 평가한다 (실제 데이터 없음).
