"""11. 그림 8장. 형식을 서로 다르게 간다. 색·폰트만 사이트 톤으로 바꾸면 되도록 데이터·축은 스크립트가 확정한다."""
...
import numpy as np
import pandas as pd
from config import DATA, EMOTIONS, FIGURES, PHQ_CUTOFF, RESULTS, SEED, TASK_KO, TASKS
...
# Noto CJK가 없는 macOS에서는 AppleGothic으로
plt.rcParams["font.family"] = ["Noto Sans CJK JP", "AppleGothic", "DejaVu Sans"]
...
cols = [C_GRAY, C_NEG, C_GRAY, C_NEG]
bars = ax2.bar(cats, vals, color=cols)
for b, v in zip(bars, vals):
    ax2.text(b.get_x() + b.get_width() / 2, v + 8, str(int(v)), ha="center", fontsize=9)
ax2.set_title("그룹 ≠ 라벨: 어긋나는 칸")
ax2.set_ylabel("참가자 수")
ax2.spines[["top", "right"]].set_visible(False)
ax2.tick_params(axis="x", labelsize=8)
plt.tight_layout()
plt.savefig(FIGURES / "fig1_cohort.png")
plt.close()

# ---------------------------------------------------------------- fig2: 분할 단위 화살표 (녹음 랜덤 → 참가자), 모델별
...
models = [
    ("text", "텍스트", C_TEXT),
    ("audio_gbm", "음성 GBM", C_AUDIO),
    ("audio_knn", "음성 k-NN", "#3f8783"),
    ("fusion", "융합", C_FUSION),
]
fig, ax = plt.subplots(figsize=(9, 4.3))
for i, (m, name, c) in enumerate(models[::-1]):
    a = gv("recording_random", m)
    b = gv("participant", m)
    ax.annotate("", xy=(b, i), xytext=(a, i), arrowprops=dict(arrowstyle="-|>", color=c, lw=2.5, mutation_scale=15))
    ax.scatter([a], [i], color=c, s=55, zorder=3, edgecolor="white", lw=1)
    ax.set_xlim(0.83, 1.03)
    if a - b < 0.01:  # 화살표가 짧으면 라벨을 좌우로 벌린다
        ax.text(
            a + 0.004,
            i,
...
ax.set_title("녹음 단위 랜덤 분할이 부풀리는 폭 (가상데이터)")
ax.spines[["top", "right", "left"]].set_visible(False)
plt.tight_layout()
plt.savefig(FIGURES / "fig2_split_leakage.png")
plt.close()

# ---------------------------------------------------------------- fig3: 과제 × 모달 히트맵 + 집계 범위 막대
...
im = ax1.imshow(piv.values, cmap="BuPu", vmin=0.5, vmax=0.9, aspect="auto")
...
names = {
    ...
    "narrative_5": "서술 5과제",
    "memory_2": "기억 2과제",
    "fixed_text_3": "고정텍스트 3과제",
}
agg = agg.set_index("aggregation").reindex(["all_8_tasks", "narrative_5", "memory_2", "fixed_text_3"])
ax2.barh([names[i] for i in agg.index][::-1], agg.par_auc[::-1], color=[C_FUSION, C_FUSION, C_FUSION, C_GRAY][::-1])
...
ax2.set_title("어느 과제를 모아 채점하나")
ax2.spines[["top", "right"]].set_visible(False)
plt.tight_layout()
plt.savefig(FIGURES / "fig3_task_modality.png")
plt.close()

# ---------------------------------------------------------------- fig4: 라벨 정의 — 관해 환자 확률 분포 (violin/strip)
...
for ax, trained, title in [(axes[0], "group_label", "환자군 여부로 학습"), (axes[1], "phq_label", "PHQ-9 라벨로 학습")]:
    for i, (g, lab, name) in enumerate(groups):
        ...
        col = C_NEG if lab == 1 else (C_GRAY if g == "control" else C_DARK)
        ...
        ax.text(i, -0.06, name, ha="center", fontsize=8.5)
    ax.axhline(0.5, color=C_DARK, ls="--", lw=0.8)
    ax.set_xlim(-0.5, 2.5)
...
fig.suptitle("라벨을 무엇으로 두느냐: 관해 환자를 우울로 보는가")
...
# 음색 계열 피처(f0 평균 + MFCC 1~6)만으로 PCA. 화자 정체성이 실리는 축을 본다.
...
axes[0].set_title("음색 계열 피처 PCA — 참가자별 색 (25명 × 8건)")
...
im = ax1.imshow(cmn.values, cmap="BuPu", vmin=0, vmax=1)
...
ax2.plot(xs, z[0] * xs + z[1], color=C_DARK, lw=1.5)
tr = pd.read_csv(R / "emotion_transfer.csv")
rho = tr[(tr.signal.str.contains("transfer")) & (tr.level == "participant")].spearman_phq.iloc[0]
auc = tr[(tr.signal.str.contains("transfer")) & (tr.level == "participant")].auc.iloc[0]
ax2.set_xlabel("PHQ-9")
ax2.set_ylabel("감정 모델의 부정 정서 점수\nP(슬픔)+P(불안)+P(상처)")
ax2.set_title(f"전이: 감정 점수 vs PHQ-9\nSpearman {rho:.2f}, 참가자 AUC {auc:.2f}")
ax2.spines[["top", "right"]].set_visible(False)
plt.tight_layout()
...
def stem(f):
    parts = [t.split("/")[0] for t in f.split()]
    return " ".join(parts)
...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 4.6), gridspec_kw={"width_ratios": [1.5, 1]})
ax1.barh(range(len(tf))[::-1], tf.coef, color=[KC.get(k, "#888") for k in tf.kind])
...
for i, (name, full, masked) in enumerate(pairs[::-1]):
    ax2.plot([full, masked], [i, i], color=C_GRAY, lw=2, zorder=1)
    ax2.scatter([full], [i], color=C_NEG, s=90, zorder=2, label="원본" if i == 0 else None)
    ax2.scatter([masked], [i], color=C_TEXT, s=90, zorder=2, label="치료어 마스킹" if i == 0 else None)
    ax2.text(full - 0.002, i, f"{full:.3f}", ha="right", va="center", fontsize=8, color=C_NEG)
    ax2.text(masked + 0.002, i, f"{masked:.3f}", ha="left", va="center", fontsize=8, color=C_TEXT)
ax2.set_yticks(range(2))
ax2.set_yticklabels([p[0] for p in pairs[::-1]])
ax2.set_xlabel("PHQ 기준 참가자 AUC")
ax2.set_title("치료어를 지워도 남는다")
ax2.legend(fontsize=8, loc="lower center")
ax2.spines[["top", "right"]].set_visible(False)
ax2.set_xlim(0.865, 0.955)
ax2.set_ylim(-0.6, 1.6)
plt.tight_layout()
plt.savefig(FIGURES / "fig7_text_features.png")
plt.close()

# ---------------------------------------------------------------- fig8: 하위군 forest + 보정 곡선
...
rows_ = []
for dim, sub, name in order:
    r = sg[(sg.dimension == dim) & (sg.subgroup == sub)]
    if len(r):
        rows_.append((name, r.auc.iloc[0], r.ci_low.iloc[0], r.ci_high.iloc[0], int(r.n.iloc[0])))
for i, (name, a, lo, hi, n) in enumerate(rows_[::-1]):
    ax1.plot([lo, hi], [i, i], color=C_DARK, lw=1.5)
    ax1.scatter([a], [i], color=C_FUSION, s=70, zorder=3)
    ax1.text(hi + 0.004, i, f"{a:.2f}", va="center", fontsize=8)
    ax1.text(0.79, i, f"{name} (n={n})", va="center", ha="left", fontsize=9)
ax1.axvline(sg[sg.subgroup == "all"].auc.iloc[0], color=C_GRAY, ls="--", lw=1)
ax1.set_xlim(0.79, 1.0)
ax1.set_ylim(-0.5, len(rows_) - 0.5)
ax1.set_yticks([])
ax1.set_xlabel("참가자 단위 AUC (95% CI)")
ax1.set_title("융합 모델 하위군 성능")
ax1.spines[["top", "right", "left"]].set_visible(False)
cal = pd.read_csv(R / "calibration_deciles.csv")
ax2.plot([0, 1], [0, 1], color=C_GRAY, ls="--", lw=1)
ax2.plot(cal.mean_pred, cal.observed, color=C_FUSION, marker="o", ms=6)
...
ax2.set_title("보정 곡선 (숫자 = 분위 평균 PHQ-9)")
ax2.spines[["top", "right"]].set_visible(False)
ax2.set_xlim(0, 1)
ax2.set_ylim(0, 1)
plt.tight_layout()
plt.savefig(FIGURES / "fig8_subgroup_calibration.png")
plt.close()
print("figures written:", len(list(FIGURES.glob("*.png"))))
