...
# ---- D1 (정신건강진단 및 예측을 위한 멀티모달 데이터) schema를 따르는 코호트 ----
N_PARTICIPANTS = 2000
...
PHQ_PATIENT = (12.5, 5.5)  # 환자군 PHQ-9 (mean, sd). 치료 중 remission이 섞임
...
# ---- 가상데이터에 넣은 구조. 신호 강도는 원본 프로젝트의 텍스트/음성 AUC 수준에 맞춰 조정한 값 ----
TEXT_SIGNAL = 1.0  # PHQ z가 free-speech task 문장 선택에 미치는 강도
AUDIO_SIGNAL = 0.28  # PHQ z점수가 음향 feature에 미치는 강도 (표준편차 단위)
MFCC_NOISE_SD = 0.35  # 음색 feature 녹음 간 잡음. 작아서 화자 안에서 안정적 = speaker leakage 경로
PROSODY_NOISE_SD = 1.0  # 운율, 에너지 feature 녹음 간 잡음
SPEAKER_FP_SD = 1.0  # speaker identity 신호 크기 (sd 단위). speaker leakage의 원천
P_TREATMENT_MENTION = {"patient": 0.60, "control": 0.03}  # 치료 맥락 언급 확률. 환자군 shortcut의 원천
P_STT_ERROR = 0.04  # STT 오인식으로 어절이 바뀌는 확률
P_IDIOLECT = 0.55  # free-speech task에서 참가자 고유 말버릇이 나올 확률 (텍스트 쪽 speaker leakage 경로)

# ---- D2 (감성 대화 말뭉치) schema를 따르는 감정 코퍼스 ----
