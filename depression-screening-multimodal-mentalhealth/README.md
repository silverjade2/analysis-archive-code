...
원본(2024 상반기)은 번아웃을 겪는 직장인이 자기 발화로 우울을 조기에 알아차리게 하는 서비스용으로, AI-Hub 공개 데이터 두 개로 한국어 우울 분류 모델과 감정 분류 모델을 만든 프로젝트. 여기서는 그 데이터를 쓰지 않고 스키마만 가져와 같은 모양의 가상 데이터를 만든 뒤, 스크리닝 파이프라인을 처음부터 다시 돌려 모델이 무엇을 배우는지 본다 (실제 데이터 없음. 원본 D1은 지정 분석 환경에서만 접근 가능한 보건의료 데이터).
