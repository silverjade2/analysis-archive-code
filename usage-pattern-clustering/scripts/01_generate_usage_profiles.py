...
클러스터링은 정답이 없어 결과가 틀려도 티가 나지 않는다. 그래서 정답을 심었다. 평일 아침 6시에서 10시 피크,
종일 낮은 강도, 21시에서 새벽 2시 피크, 평소 거의 0이고 특정 시간대에만 고강도 버스트, 이 넷이 진짜 군집이다.
여기에 함정을 얹었다. total_usage만 0에서 5000 스케일이라 scaling 없이는 이 축이 거리를 지배하고, night와
