# 클러스터 해석: 최종 군집(로그+표준화, k=5)의 파생 피처 z-score 프로파일링과 비즈니스 라벨링, 정답 레이블 사후 검증
# 출력: outputs/figures/fig7_cluster_zscore.png, outputs/results/cluster_zscore.csv·cluster_labels.csv·final_crosstab.csv
...
# 해석용 파생 피처: 168개 셀은 읽을 수 없으므로 사람 단위(시간대 구간, 주말 비율, 버스트성)로 접어 z-score화
...
# 비즈니스 라벨: 히트맵을 읽고 손으로 붙임, z-score를 비즈니스 언어로 번역하는 판단 작업이라 자동화하지 않음
