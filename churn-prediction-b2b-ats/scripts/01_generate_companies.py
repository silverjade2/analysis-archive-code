...
라벨은 여기서 만들지 않는다. 03이 원본 규칙으로 도출한다. 갱신 확률의 진짜 원인은 여섯이다. 인원 성장률, 연간
채용 비율, 두 상품 동시 사용, 재직 인원은 갱신을 올리고 업력과 퇴사 비율은 내린다. 외부데이터는 건강보험과 공시를
흉내낸 것인데 기업 규모가 클수록 존재 확률을 높게 심었다. 결합 탈락이 작은 회사에 몰리게 하려는 것이다. 계약
이력은 DATA_END인 2026-06까지 만들어 두고 스냅샷 절단은 쓰는 쪽에서 한다. 06의 기준일 sweep이 미래 계약을
필요로 하기 때문이다.
"""
