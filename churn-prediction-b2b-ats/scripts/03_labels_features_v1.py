"""원본 노트북 구조의 라벨과 feature 테이블. v1 스냅샷.

라벨은 실행일 기준으로 최종 종료일에서 90일이 지났으면 이탈. 원본의 Churn() 그대로다. 파생은 최초 계약일, 신규와
재계약 구분(최초 계약 연월과 같으면 신규), 사용 유무. feature 30열은 원본 노트북의 data_encoded와 같은 구성이다.
원본은 연도별 매출 2018~2023을 드롭하고 2015와 2024~2027을 남겼다. 이상하지만 그대로 재현한다.
"""

import numpy as np
import pandas as pd
from common import CHURN_GRACE_DAYS, DATA, REF_DATE, RESULTS, churn_label
...
snap["gubun"] = np.where(
    (snap.contract_date.dt.year == snap.first_contract.dt.year)
    & (snap.contract_date.dt.month == snap.first_contract.dt.month),
    "신규",
    "재계약",
)
snap["year"] = snap.start_date.dt.year
last_row = snap.sort_values("end_date", ascending=False).drop_duplicates("company_id")[
    ["company_id", "gubun", "first_contract"]
]
...
num = df[
    [
        ...
        "rev_2025",
        "rev_2026",
        "rev_2027",  # 2018~2023은 드롭, 2015와 2024~2027은 유지. 원본 그대로
        "n_contracts",
        "total_revenue",
        "first_year",
        "first_month",
...
stats = [
    ...
    ("churn_rate", X.y.mean()),
    ("in_use_rate", df.in_use.mean()),
    ("gubun_new_rate", (df.gubun == "신규").mean()),
]
fut = []
for c in ["rev_2015", "rev_2024", "rev_2025", "rev_2026", "rev_2027", "n_contracts", "total_revenue"]:
    ...
    fut.append(
        dict(
            ...
            churn_rate_if_positive=df.y[pos].mean() if pos.any() else np.nan,
...
by_year = (
    df.groupby("first_year")
    .agg(n=("company_id", "size"), churn_rate=("y", "mean"), mean_renewal_opportunities=("exposure_k", "mean"))
    .reset_index()
)
...
fut.append(
    dict(
        feature="gubun_신규",
        n_positive=int((df.gubun == "신규").sum()),
        churn_rate_if_positive=df.y[df.gubun == "신규"].mean(),
        churn_rate_if_zero=df.y[df.gubun != "신규"].mean(),
    )
)
pd.DataFrame(fut).round(4).to_csv(RESULTS / "future_revenue_leak.csv", index=False)
print(pd.DataFrame(stats))
print(pd.DataFrame(fut).round(3))
