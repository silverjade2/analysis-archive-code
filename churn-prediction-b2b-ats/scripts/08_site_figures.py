...
for i, v in enumerate(bb.kept_rate):
    ax[0].text(i, v + 0.02, f"{v:.0%}", ha="center", fontsize=9)
ax[0].set_ylim(0, 1.08)
ax[0].set_title("외부데이터 결합 후 남은 비율 (재직인원 구간별)")
ax[0].set_ylabel("결합 성공률")
x = np.arange(len(bb))
w = 0.38
ax[1].bar(x - w / 2, bb.churn_all, w, color=GRAY, label="결합 전 전체")
ax[1].bar(x + w / 2, bb.churn_kept, w, color=BLUE, label="결합 후 남은 회사")
ax[1].set_xticks(x)
ax[1].set_xticklabels(bb.size_band)
ax[1].set_ylim(0, 0.75)
ax[1].axhline(s.churn_rate_all, color=GRAY, ls="--", lw=1)
ax[1].axhline(s.churn_rate_kept, color=BLUE, ls="--", lw=1)
...
ax[1].set_title("이탈률: 탈락 전후")
ax[1].legend(fontsize=8, loc="upper right")
fig.savefig(FIGURES / "fig1_join_bias.png")
plt.close(fig)

by = R("churn_by_first_year.csv")
fig, ax = plt.subplots(figsize=(8, 3.6))
ax.bar(by.first_year.astype(str), by.churn_rate, color=GRAY)
for i, (v, n) in enumerate(zip(by.churn_rate, by.n)):
    ax.text(i, v + 0.02, f"{v:.0%}\n(n={n})", ha="center", fontsize=8)
ax.set_ylim(0, 1.0)
ax.set_ylabel("이탈률 (2024-02 기준)")
ax.set_xlabel("최초 계약 연도")
ax2 = ax.twinx()
ax2.plot(by.first_year.astype(str), by.mean_renewal_opportunities, color=ORANGE, marker="o")
...
fl = R("future_revenue_leak.csv")
fl = fl[fl.feature.isin(["rev_2015", "rev_2024", "rev_2025", "n_contracts", "total_revenue", "gubun_신규"])]
lab = {
    "rev_2015": "2015년 매출 > 0",
    "rev_2024": "2024년 매출 > 0",
    "rev_2025": "2025년 매출 > 0",
    "n_contracts": "총계약수 > 중앙값",
    "total_revenue": "총매출 > 중앙값",
    "gubun_신규": "구분 = 신규 (재계약 이력 없음)",
}
...
ax.set_yticks(y)
ax.set_yticklabels([lab[f] for f in fl.feature])
ax.invert_yaxis()
ax.set_xlim(0, 1)
ax.set_xlabel("이탈률")
ax.legend(fontsize=8, loc="lower right")
ax.set_title("feature별 target rate")
fig.savefig(FIGURES / "fig3_future_revenue_leak.png")
plt.close(fig)

mc = R("model_compare.csv")
cls = mc[mc.task == "classification"]
...
ax.hlines(o1141, -0.3, 1.3, color=GREEN, ls="--")
ax.text(1.3, o1141 + 0.004, f"oracle(1,141) {o1141:.3f}", color=GREEN, fontsize=8, ha="right")
...
live = sw[sw["mode"] == "live"].set_index("ref_date")
frz = sw[sw["mode"] == "frozen"].set_index("ref_date")
post = live.index >= REF_DATE
fig, (a0, a1) = plt.subplots(
    2, 1, figsize=(9, 5.6), sharex=True, gridspec_kw={"height_ratios": [2.2, 1], "hspace": 0.12}
)
for a in (a0, a1):
    a.grid(axis="x", visible=False)
    a.spines["left"].set_color("#cfd3d9")
    a.spines["bottom"].set_color("#cfd3d9")
...
a0.plot(live.index, frz.churn_rate, color=RED, lw=2.2)
...
for months, dy in [(6, -10), (12, -10), (24, -10)]:
    ...
    for ser, col in [(frz, RED), (live, BLUE)]:
        ...
        a0.text(
            d,
            ser.churn_rate[d] + (0.028 if col == RED else -0.05),
            f"{ser.churn_rate[d]:.0%}",
            fontsize=8,
            ha="center",
            color=col,
        )
...
a1.text(
    live.index.max() + pd.Timedelta(days=12),
    live.flip_rate.iloc[-1],
    f"{live.flip_rate.iloc[-1]:.0%}",
    fontsize=8.5,
    color=BLUE,
    va="center",
)
...
a1.set_xlabel("기준일 (as-of date, 라벨의 today)", fontsize=9)
fig.suptitle("기준일(as-of date)에 따른 이탈률 (같은 회사, 같은 규칙)", x=0.125, ha="left", fontsize=11.5, y=0.955)
fig.text(
    0.125,
    0.905,
...
axt = fig.add_subplot(gs[1, 0], sharex=ax0)
...
for prod, (col, lab_, yv) in ys:
    yy = yv
    if placed and yy - placed[-1] < 0.045:
        yy = placed[-1] + 0.046
    placed.append(yy)
    ax0.text(XMAX + 1.2, yy, f"{lab_}  {yv:.0%}", fontsize=8.5, color=col, va="center")
for m in (12, 24, 36, 48):
    ax0.axvline(m, color="#cfd3d9", lw=0.7, ls=(0, (2, 3)), zorder=0)
ax0.set_xlim(0, XMAX)
ax0.set_ylim(0.25, 1.0)
ax0.yaxis.set_major_formatter(PercentFormatter(1.0, decimals=0))
ax0.set_ylabel("계약 유지 확률 (Kaplan-Meier, 95% CI)", fontsize=9)
ax0.tick_params(labelbottom=False)
ax0.set_title("상품 구성별 Kaplan-Meier survival curve", loc="left", fontsize=10.5)
ax0.set_xticks([0, 12, 24, 36, 48, 60])
axt.set_ylim(-0.5, 2.9)
axt.set_yticks([])
axt.grid(False)
for sp in axt.spines.values():
    sp.set_visible(False)
axt.tick_params(length=0, labelsize=9)
axt.set_xlabel("최초 계약 후 개월", fontsize=9)
...
names = {
    "headcount_growth": "인원 성장률 (+1 SD)",
    "log_hiring_rate": "채용 비율 log (+1 SD)",
    "log_attrition_rate": "퇴사 비율 log (+1 SD)",
    "log_employees": "재직 인원 log (+1 SD)",
    "firm_age": "업력 (+1 SD)",
    "product_both": "상품 A+B (vs A만)",
    "product_posting": "상품 B만 (vs A만)",
}
...
for i, r in cx.iterrows():
    sig = r["p"] < 0.05
    col = "#9aa0a8" if not sig else (BLUE if r["exp(coef)"] < 1 else ORANGE)
    ...
    ax1.text(
        1.03,
        i,
        f"{r['exp(coef)']:.2f}  [{r['exp(coef) lower 95%']:.2f}, {r['exp(coef) upper 95%']:.2f}]",
        transform=tr,
        fontsize=8,
        va="center",
        color="#444",
...
ax1.axvline(1, color="#555", lw=0.9)
ax1.set_xscale("log")
ax1.set_xlim(0.38, 1.75)
ax1.set_xticks([0.5, 0.7, 1.0, 1.4])
ax1.set_xticklabels(["0.5", "0.7", "1.0", "1.4"])
ax1.minorticks_off()
...
cal = R("calibration.csv")
pts = cal.dropna(subset=["mean_pred"])
...
ax.set_xlabel("Predicted churn probability (LightGBM OOF, 8 bins)")
