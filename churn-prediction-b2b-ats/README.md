...
원본은 실제 B2B ATS 고객사 데이터로 했던 이탈 예측. 같은 schema로 가상 고객사 2000사를 만들고, 라벨 기준일을 고정한 설계(v2)와 고정하지 않은 대조군(v1)을 같은 절차로 돌려 비교한다. 평가 기준은 생성기가 넣어 둔 갱신 확률 `truth_renew_p`. 실제 데이터는 없음.
...
3분쯤, 05가 2분. `verify.py`는 feature에 `truth_*`가 섞이지 않았는지, T cutoff 손검산 5사, v2 AUC가 oracle을 넘지 않는지 확인.

Python 3.12, pandas 2.3, scikit-learn 1.8, LightGBM 4.7, XGBoost 3.4, shap 0.52, lifelines 0.30 (Linux x86)에서 만든 결과. Apple Silicon이나 다른 버전에서는 `model_compare.csv`의 XGBoost/RF 행이 셋째 자리에서 흔들린다 (LightGBM, LR, oracle은 같음). 글의 AUC 범위(v2 0.835~0.859)는 그 값도 인용하니 커밋된 CSV 기준으로 볼 것.

## 스크립트

| | 역할 | 주요 출력 |
| --- | --- | --- |
| 01 | 고객사 2000사, 계약 이력, 외부데이터 생성. 갱신 확률 `truth_renew_p`와 join 탈락 selection bias를 넣음 | `data/companies.csv`, `contracts.csv`, `external.csv` |
...
| 04 | 설계. T(기준일 12개월 전) cutoff feature + oracle | `features_v2.csv`, `truth_v2_population.csv` |
...
- 03: 이탈률 0.392
- 05: v1(대조군) LightGBM AUC 0.978 > oracle(1141) 0.876. oracle보다 높으면 leakage. v2(설계) LightGBM 0.852 < oracle(688) 0.871
- 05: v2에서 external.csv 13열을 빼면 LightGBM 0.852 -> 0.801, LR 0.835 -> 0.799. v2 SHAP 비중은 외부 13열 합 40%
- 07: concordance 0.664. Cox 계수 부호 6개가 넣어 둔 방향과 전부 일치

## 데이터

`companies.csv` 2000행, `contracts.csv` 6682행 (다년 계약은 연 단위 세금계산서 행으로 split), `external.csv` 1201행 (규모 클수록 존재. 매출/영업이익 56.5% 결측, 연봉 31.6% 결측, 단위 오류 1건). 계약 체결일은 2026-06(`DATA_END`)까지, 다년 계약 종료일은 2029-06까지 생성돼 있고 snapshot cutoff는 쓰는 쪽에서 `contract_date <= 기준일`로 한다.
...
- `external_ablation.csv`의 제외 행은 macOS(Python 3.14, scikit-learn 1.9)에서 만든 값. 포함 쪽 AUC는 `model_compare.csv`의 v2 행을 그대로 쓰고, XGBoost/RF는 Linux에서 셋째 자리가 다를 수 있음
